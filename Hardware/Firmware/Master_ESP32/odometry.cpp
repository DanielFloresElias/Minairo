...
#include "odometry.h"
 ...
 // Constructor
Odometry::Odometry(float ticks_per_rev, float r, float D)
  : ticks_per_rev(ticks_per_rev), r(r), D(D), x(0.0), y(0.0), theta(0.0) {}

// Método para resetear las variables de posición a cero
...
_ODOM Odometry::calculate_odometry(int encoder_vals[], int prev_encoder_vals[], unsigned long InstantTime, unsigned long prev_InstantTime) {
  ...
  float theta_1 = radians(60);
  float theta_2 = radians(180);
  ...
  float vx = (wheel_velocities[0] * _cos30 - wheel_velocities[1] * _cos30) * 2.0 / 3.0;                // Velocidad en X
  float vy = (wheel_velocities[0] * 0.5 + wheel_velocities[1] * 0.5 - wheel_velocities[2]) * 2 / 3.0;  // Velocidad en Y
  float omega = (wheel_velocities[0] + wheel_velocities[1] + wheel_velocities[2]) / (3 * D);           // Velocidad angular

  // Cinemática directa: calcular la velocidad en los ejes X, Y y angular (omega)
  //float vx = (-wheel_velocities[0] * _cos30 + wheel_velocities[1] * _cos30) / 2.0;    // Velocidad en X
  //float vy = (wheel_velocities[0] * 0.5 + wheel_velocities[1] * 0.5 - wheel_velocities[2]) / 2.0;      // Velocidad en Y
  //float omega = (wheel_velocities[0] + wheel_velocities[1] + wheel_velocities[2]) / (3 * D);          // Velocidad angular
  ...
  _ODOM odometry = { x, y, theta, velocity_linear, direction_angle };
  return odometry;
}
/*
